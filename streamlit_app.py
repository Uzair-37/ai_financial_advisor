...
Educational tone, 150 words max."""
    ...
    def _fallback_analysis(self, prompt):
        ...
        # Extract market data from prompt for contextual analysis
        if "stock analysis" in prompt.lower():
    ...
    def _generate_stock_insights(self, prompt):
        ...
        lines = prompt.split('\n')
        analysis = "📊 **Stock Analysis Insights:**\n\n"
        ...
        for line in lines:
            ...
            elif "Price Movement:" in line:
                movement = line.split(':')[1].strip()
                if '+' in movement:
                    analysis += f"📈 **Positive Momentum**: Stock has gained {movement}, showing upward pressure.\n\n"
                elif '-' in movement:
                    analysis += f"📉 **Declining Trend**: Stock has dropped {movement}, indicating selling pressure.\n\n"
            elif "RSI:" in line:
                rsi_val = line.split(':')[1].strip()
                try:
                    rsi_num = float(rsi_val)
                    if rsi_num > 70:
                        analysis += f"⚠️ **Overbought Signal**: RSI of {rsi_val} suggests the stock may be overextended.\n\n"
                    elif rsi_num < 30:
                        analysis += f"🟢 **Oversold Signal**: RSI of {rsi_val} indicates potential buying opportunity.\n\n"
                    else:
                        analysis += f"⚖️ **Neutral Momentum**: RSI of {rsi_val} shows balanced buying/selling pressure.\n\n"
                except:
                    pass
            elif "Trend:" in line:
                trend = line.split(':')[1].strip()
                if trend == "Bullish":
                    analysis += "🚀 **Bullish Pattern**: Price above key moving averages suggests upward momentum.\n\n"
                elif trend == "Bearish":
                    analysis += "🔻 **Bearish Pattern**: Price below moving averages indicates downward pressure.\n\n"
                else:
                    analysis += "➡️ **Sideways Action**: Mixed signals suggest consolidation phase.\n\n"
        
        analysis += "💡 **Investment Insight**: Consider these signals together with broader market conditions and your risk tolerance."
        return analysis
